...
post '/location/:venue/:untappd' do
  untappd_id = params[:untappd]
  fsq_id = params[:venue]
  ...
  begin
    ...
    venue = @loc_db.get(fsq_id)
      ...
      puts "No database record found, pulling from foursquare"
      @loc_db.save_doc({'_id' => fsq_id, :name => location['name'], :twitter => location['contact']['twitter'], :fb_id => location['contact']['facebook'], :untappd_id => untappd_id})
      venue = @loc_db.get(fsq_id)
  rescue Foursquare2::APIError
      puts "Venue not found for #{fsq_id} please verify id from Foursquare"
      halt 404
